...
function ShipmentsList() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (q)
        query = query.or(
          `tracking_number.ilike.%${q}%,receipt_number.ilike.%${q}%,sender_name.ilike.%${q}%,receiver_name.ilike.%${q}%`,
        );
      ...
      <div className="mb-4 flex max-w-md gap-2">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="Search tracking, receipt, sender, receiver…"
            className="pl-9"
          />
        </div>
      </div>
            ...
            {(data ?? []).map((s) => (
              <tr key={s.id} className="border-t border-border hover:bg-muted/30">
                <td className="px-5 py-3 font-mono text-xs">
                  <Link to="/shipments/$id" params={{ id: s.id }} className="hover:text-pme-red">
                    {s.tracking_number}
                  </Link>
                ...
                <td className="px-5 py-3 text-muted-foreground">
                  {s.sender_city} → {s.destination_city}, {s.destination_country}
                </td>
                <td className="px-5 py-3">
                  <span
                    className={`rounded-full border px-2 py-0.5 text-xs font-medium ${statusBadgeClass(s.current_status)}`}
                  >
