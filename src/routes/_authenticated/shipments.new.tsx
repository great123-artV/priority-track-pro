...
function NewShipment() {
  ...
  const { data: branches } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("branches").select("id,name,city,country").order("name")).data ?? [],
  });
  ...
  const [f, setF] = useState({
    ...
    registration_charge: 25,
    custom_clearance_charge: 0,
    insurance_fee: 0,
    handling_fee: 10,
    discount: 0,
    currency: "USD",
    payment_method: "Cash",
    payment_status: "pending" as "pending" | "paid" | "partial" | "refunded",
  });

  const total = useMemo(
    () =>
      Number(f.registration_charge) +
      Number(f.custom_clearance_charge) +
      Number(f.insurance_fee) +
      Number(f.handling_fee) -
      Number(f.discount),
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const insertPayload = {
        ...
        sender_country: f.sender_country,
        ...
        origin_branch_id: f.origin_branch_id || null,
        destination_branch_id: f.destination_branch_id || null,
        destination_country: f.destination_country,
        destination_city: f.destination_city,
        departure_date: f.departure_date,
        expected_arrival_date: f.expected_arrival_date,
        special_handling_note: f.special_handling_note,
        declared_value: Number(f.declared_value),
        insurance_required: f.insurance_required,
        registration_charge: Number(f.registration_charge),
        custom_clearance_charge: Number(f.custom_clearance_charge),
        insurance_fee: Number(f.insurance_fee),
        handling_fee: Number(f.handling_fee),
  ...
  return (
    <div className="container mx-auto max-w-5xl px-6 py-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Fill in the details below. AWB number and receipt number are generated automatically.
      </p>
      ...
      <form onSubmit={submit} className="mt-6 space-y-6">
        <Section title="Sender Information">
          <Grid>
            ...
            <F label="Phone">
              <Input value={f.sender_phone} onChange={(e) => set("sender_phone", e.target.value)} />
            </F>
            <F label="Email">
              <Input
                type="email"
            ...
            <F label="Country">
              <Input
                value={f.sender_country}
                onChange={(e) => set("sender_country", e.target.value)}
              />
            </F>
            <F label="City">
              <Input value={f.sender_city} onChange={(e) => set("sender_city", e.target.value)} />
            </F>
        ...
        <Section title="Shipment Information">
          <Grid>
            <F label="Package Description" full>
              <Input
                value={f.package_description}
                onChange={(e) => set("package_description", e.target.value)}
              />
            </F>
            <F label="Contents">
              <Input
                value={f.package_contents}
                onChange={(e) => set("package_contents", e.target.value)}
              />
            </F>
            <F label="Quantity">
              <Input
            ...
            <F label="Destination Branch">
              <Select
                value={f.destination_branch_id}
                onValueChange={(v) => set("destination_branch_id", v)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select" />
                </SelectTrigger>
            ...
            <F label="Declared Value">
              <Input
                ...
                onChange={(e) => set("declared_value", Number(e.target.value))}
              />
            </F>
            <F label="Insurance Required">
              <div className="flex h-10 items-center gap-3">
                <Switch
                  checked={f.insurance_required}
                  onCheckedChange={(v) => set("insurance_required", v)}
                />
                <span className="text-sm text-muted-foreground">
                  {f.insurance_required ? "Yes" : "No"}
                </span>
              </div>
            </F>
            <F label="Special Handling Note" full>
              <Textarea
                rows={2}
                value={f.special_handling_note}
                onChange={(e) => set("special_handling_note", e.target.value)}
              />
            </F>
          </Grid>
        </Section>
        ...
        <Section title="Payment Information">
          <Grid>
            <F label="Registration Charge">
              <Input
                type="number"
                step="0.01"
                value={f.registration_charge}
                onChange={(e) => set("registration_charge", Number(e.target.value))}
              />
            </F>
            <F label="Custom Clearance">
              <Input
                type="number"
                step="0.01"
                value={f.custom_clearance_charge}
                onChange={(e) => set("custom_clearance_charge", Number(e.target.value))}
              />
            </F>
            <F label="Insurance Fee">
              <Input
                type="number"
                step="0.01"
                value={f.insurance_fee}
                onChange={(e) => set("insurance_fee", Number(e.target.value))}
              />
            </F>
            <F label="Handling Fee">
              <Input
                type="number"
                step="0.01"
                value={f.handling_fee}
                onChange={(e) => set("handling_fee", Number(e.target.value))}
              />
            </F>
            <F label="Discount">
              <Input
            ...
            <F label="Payment Status">
              <Select
                ...
                onValueChange={(v) => set("payment_status", v as typeof f.payment_status)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="flex items-end md:col-span-2">
              <div className="rounded-lg border-2 border-pme-red/30 bg-pme-red/5 p-4 text-right">
                ...
                <div className="text-display text-2xl font-bold text-pme-red">
                  {f.currency} {total.toFixed(2)}
                </div>
              </div>
            </div>
          </Grid>
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => navigate({ to: "/shipments" })}>
            Cancel
          </Button>
          <Button
            type="submit"
          ...
          >
            {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null} Create Shipment &
            Generate Receipt
          </Button>
        </div>
      </form>
...
function F({
  ...
  return (
    <div className={`space-y-1.5 ${full ? "md:col-span-2 lg:col-span-3" : ""}`}>
      <Label>{label}</Label>
      {children}
    </div>
  );
