...
function MovementSearch() {
  ...
  const search = async (e: React.FormEvent) => {
    ...
    const { data } = await supabase
      .from("shipments")
      .select("id")
      .or(`tracking_number.eq.${code.trim()},receipt_number.eq.${code.trim()}`)
      .maybeSingle();
    setBusy(false);
    if (!data) {
  ...
  return (
    <div className="container mx-auto max-w-2xl px-6 py-12">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Search by tracking or receipt number to add a movement event.
      </p>
      <form
        onSubmit={search}
        className="mt-6 flex gap-2 rounded-xl border border-border bg-card p-3 shadow-card"
      >
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="PME-AWB-... or PME-RCP-..."
            className="h-11 pl-9"
          />
        </div>
        <Button
