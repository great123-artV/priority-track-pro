...
function Branches() {
  ...
  const [f, setF] = useState({ name: "", country: "", city: "", address: "", manager_name: "", phone: "", status: "active" });

  const save = async () => {
    if (!f.name || !f.country || !f.city) { toast.error("Name, country, and city required"); return; }
    const { error } = await supabase.from("branches").insert(f);
    if (error) { toast.error(error.message); return; }
    toast.success("Branch added");
    setOpen(false); setF({ name: "", country: "", city: "", address: "", manager_name: "", phone: "", status: "active" });
    ...
    <div className="container mx-auto px-6 py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="bg-pme-red text-pme-red-foreground hover:bg-pme-red/90"><Plus className="mr-2 h-4 w-4" /> New Branch</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Add Branch</DialogTitle></DialogHeader>
            <div className="grid gap-3 md:grid-cols-2">
              {(["name", "country", "city", "manager_name", "phone"] as const).map((k) => (
                <div key={k}><Label className="capitalize">{k.replace("_", " ")}</Label><Input value={f[k]} onChange={(e) => setF({ ...f, [k]: e.target.value })} /></div>
              ))}
        ...
        {(data ?? []).map((b) => (
          <div key={b.id} className="rounded-xl border border-border bg-card p-5 shadow-card">
            <div className="mb-3 flex items-start justify-between">
              <div className="grid h-10 w-10 place-items-center rounded-lg bg-navy/10 text-navy"><Building2 className="h-5 w-5" /></div>
              <span className="rounded-full bg-success/10 px-2 py-0.5 text-xs font-medium text-success">{b.status}</span>
            </div>
