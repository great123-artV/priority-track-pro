...
function Reports() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data: rows } = await supabase
        .from("shipments")
        .select("created_at,current_status,total_amount,payment_status,origin_branch_id");
  ...
  rows.forEach((r) =>
    statusCounts.set(r.current_status, (statusCounts.get(r.current_status) ?? 0) + 1),
  );
  const statusData = Array.from(statusCounts.entries()).map(([k, v]) => ({
  ...
  const totalRevenue = rows
    ...
    .reduce((a, r) => a + Number(r.total_amount), 0);
  ...
  return (
    <div className="container mx-auto px-6 py-8">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-4">
        ...
        <KPI
          label="Delivered"
          value={rows.filter((r) => r.current_status === "delivered").length}
        />
