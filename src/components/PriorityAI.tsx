...
import {
  ...
  ChevronRight, Headset, MessageSquare, Mail, Phone, ExternalLink
} from "lucide-react";
...
export function PriorityAI() {
  ...
  return (
    <div className={cn(
      ...
      isMinimized ? "h-14 w-64" : "h-[600px] w-[400px] max-w-[calc(100vw-48px)] max-h-[calc(100vh-48px)]"
    )}>
      {/* Header */}
      <div className="flex items-center justify-between bg-navy p-4 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold leading-none">Priority AI</div>
            <div className="text-[10px] text-white/60">Virtual Logistics Assistant</div>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => setIsMinimized(!isMinimized)} className="rounded-md p-1 hover:bg-white/10">
      ...
      {!isMinimized && (
        <>
          ...
          <ScrollArea className="flex-1 p-4" viewportRef={scrollRef}>
            ...
            {(history.length === 0 || history[history.length - 1].role === "assistant") && !isLoading && (
              <div className="mt-6 flex flex-wrap gap-2">
                <QuickAction label={t("ai.actions.track")} text="I want to track my shipment" />
                <QuickAction label={t("ai.actions.services")} text="What services do you offer?" />
                <QuickAction label={t("ai.actions.times")} text="How long does delivery take?" />
                <QuickAction label={t("ai.actions.international")} text="Do you ship internationally?" />
                <QuickAction label={t("ai.actions.pricing")} text="Where can I find pricing information?" />
                <QuickAction label={t("ai.actions.branches")} text="Where are your branches located?" />
                <QuickAction label={t("ai.actions.support")} text="I need to contact support" />
                <QuickAction label={t("ai.actions.faq")} text="Show me frequently asked questions" />
              </div>
            )}
          </ScrollArea>

          {/* Support Escalation (show when support is mentioned or at bottom of FAQ) */}
          {!isLoading && history.some(m => m.content.toLowerCase().includes("support") || m.content.toLowerCase().includes("contact")) && (
            <div className="px-4 pb-2">
              <div className="rounded-xl border border-border bg-accent/5 p-3">
                <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-2">Connect with us</div>
                <div className="grid grid-cols-2 gap-2">
                  <Button asChild variant="ghost" size="sm" className="h-8 text-[10px] justify-start px-2">
                  ...
                  <Button variant="ghost" size="sm" className="h-8 text-[10px] justify-start px-2 cursor-not-allowed opacity-50">
          ...
          <div className="p-4 border-t border-border">
            ...
            >
              <Input
                ...
                disabled={isLoading}
              />
              <Button type="submit" size="icon" className="bg-pme-red hover:bg-pme-red/90" disabled={isLoading}>
